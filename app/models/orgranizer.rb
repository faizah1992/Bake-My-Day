...
def place_an_order
    ...
    browse_menu
    puts "\n"
    selected_cupcake_name = prompt.select("Which cupcake would you like to purchase".colorize(:light_magenta),all_cupcakes)
    selected_cupcake_object = Cupcake.all.find_by(name: selected_cupcake_name)
    puts "\nHow many #{selected_cupcake_name}s would you like?".colorize(:Light_blue)
    
    quantity = gets.chomp.to_i
    

    puts "\nNow we need your delivery information".colorize(:light_magenta)
